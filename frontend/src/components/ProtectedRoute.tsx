...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
  ...
  if (user && requireApproval && !user.isApproved && (user.role === 'higher-authority' || user.role === 'institute')) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
          ...
          <CardContent className="text-center space-y-4">
            <div className="text-sm text-gray-600">
              {user.role === 'higher-authority' && (
                <p>Your higher authority registration is being reviewed by administrators.</p>
              )}
              {user.role === 'institute' && (
                <p>Your institute registration is being reviewed by your higher authority.</p>
              )}
            </div>
...
export const HigherAuthorityRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => (
...
export const PublicRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <ProtectedRoute allowedRoles={['user', 'unregistered', 'super-admin', 'admin', 'higher-authority', 'institute']} requireApproval={false}>
    {children}
  </ProtectedRoute>
); 