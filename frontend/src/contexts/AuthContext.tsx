...
export type UserRole = 'super-admin' | 'admin' | 'higher-authority' | 'institute' | 'unregistered' | 'user';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkUserRole = useCallback(async (userAddress?: string, providerInstance?: ethers.BrowserProvider) => {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        if (provider) {
          await checkUserRole(userAddress, provider);
        }
      }
    } catch (error: any) {
  ...
  }, [address, provider, checkUserRole]);
